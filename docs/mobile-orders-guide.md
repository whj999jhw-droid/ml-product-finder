...
>   - **计算口径（与运营约定一致）**：自「下单时间」(`date_created`) 起 72 个营业小时；**按中国日历**——周末（中国周六日）不计入、顺延；中国法定节假日不计入、顺延（含国务院调休：如国庆 10/1-10/7 整段顺延、调休补班日 10/10 仍算工作日）。节假日数据由 `date-holidays` 库（中国）+ 内置的 2026 调休表共同提供，逐年更新。
