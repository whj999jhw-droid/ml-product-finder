...
function extractItemImage(item: any): string | undefined {
  ...
  const pics = it?.pictures || it?.itemImages || [];
  if (pics.length) return pics[0]?.secure_url || pics[0]?.url || pics[0];
  return undefined;
}

function formatOrderItems(order: any): { title?: string; quantity?: number; price?: string; image?: string }[] {
...
export async function notifyNewOrder(store: { nickname?: string; site: string }, order: any): Promise<NotifyResult> {
  ...
  // 邮件正文（纯文本）
  const itemLines = items.map((it, idx) => `${idx + 1}. ${it.title}${it.quantity ? ` x${it.quantity}` : ''}${it.price ? ` (${it.price})` : ''}`);
  const text = [
    `【美客多新订单】`,
    `店铺：${storeName}`,
    `订单号：${order.id}`,
  ...
  // Webhook/短信正文（markdown 可展示图片链接）
  const itemTexts = items.map((it, idx) => {
    const line = `${idx + 1}. ${it.title}${it.quantity ? ` x${it.quantity}` : ''}${it.price ? ` (${it.price})` : ''}`;
    return it.image ? `${line}\n![商品图](${it.image})` : line;
  });
  const smsText = [
  ...
  const itemHtml = items.map((it, idx) => {
    const img = it.image ? `<br/><img src="${escapeHtml(it.image)}" style="max-width:200px;max-height:200px;border:1px solid #eee;border-radius:4px;" />` : '';
    return `<div style="margin-bottom:12px;">${idx + 1}. ${escapeHtml(it.title)}${it.quantity ? ` x${it.quantity}` : ''}${it.price ? ` (${escapeHtml(it.price)})` : ''}${img}</div>`;
  }).join('');
  const html = [
    ...
    items.length ? `<h4 style="margin:0 0 8px;">商品：</h4>${itemHtml}` : '',
    `</div>`,
  ].filter(Boolean).join('\n');

  if (notifyConfig.orderAlertsEnabled && notifyConfig.emailEnabled) {
    results.push({ channel: 'email', ...(await sendEmailAlert(`[ML] 新订单 ${order.id} - ${storeName}`, text, html)) });
  }
  if (notifyConfig.orderAlertsEnabled && notifyConfig.smsEnabled) {
    results.push({ channel: 'sms', ...(await sendSmsAlert(smsText)) });
  }
  return { text, smsText, html, results };
}
