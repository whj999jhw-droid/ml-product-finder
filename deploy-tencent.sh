#!/bin/bash
# ==============================================================
# ML Product Finder — 腾讯云 一键部署脚本（支持中断续部署）
#
# 用法：
#   chmod +x deploy-tencent.sh
#   ./deploy-tencent.sh                 # 第一次运行：按提示填写
#   ./deploy-tencent.sh                 # 中途断网/失败后再跑：自动跳过已完成步骤
#   ./deploy-tencent.sh --reconfigure   # 强制重新填写所有配置
#   ./deploy-tencent.sh --yes           # 跳过最后的「确认开始」询问（配合 --reconfigure 可全自动）
#
# 特性：
...
trap 'echo -e "\n\033[0;31m[✗] 部署在上面的步骤中断了。\033[0m"; echo -e "    请修复上面的报错（通常是网络/权限问题），然后 \033[1;33m重新运行 ./deploy-tencent.sh\033[0m"; echo -e "    已完成的步骤会自动跳过，无需从头再来。\n"' ERR
...
echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║  ML Product Finder — 腾讯云 一键部署         ║"
echo "╚══════════════════════════════════════════════╝"
echo ""
...
if [ -z "$GIT_REPO" ]; then
  read -p "  GitHub 仓库地址 (https://github.com/你/仓库)，不用 GitHub 直接回车: " GIT_REPO
fi
if [ -z "$ML_APP_ID" ]; then
...
echo ""
echo "  配置摘要："
echo "    仓库: ${GIT_REPO:-（不使用 GitHub，将用已上传代码）}"
echo "    目录: $APP_DIR"
...
echo "║  或简单粗暴：./deploy-tencent.sh 自动处理      ║"
echo "║                                              ║"
echo "╚══════════════════════════════════════════════╝"
